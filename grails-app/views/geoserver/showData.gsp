...
<!-- <div class="crud" style="width:960px;margin-left: auto; margin-right: auto;"> -->
<div style="min-width:960px; max-width:95%; margin-left: auto; margin-right: auto;background-color: #f5f5f5; display: table;">
    ...
    <table style="width:230px;border-width: 1px; border-color: olive; ">
      ...
      <tbody>
        <tr class='even' style="border: 1px solid #888888;">
          <td class="detalle">Cobertura</td>
          <td class="fechaHora">${titulos['cobertura']}</td>
        </tr>
        ...
        <tr class='even' style="border: 1px solid #888888;">
          <td class="detalle">Registro de datos</td>
          <td class="fechaHora">${contador['n']}</td>
        </tr>
        <tr class='even' style="border: 1px solid #888888;">
          <td class="detalle">Estaciones (POHMS)</td>
          <td class="fechaHora">${(pohm != null)?pohm:contador['npohms']}</td>
        </tr>
        <tr class='even' style="border: 1px solid #888888;">
  ...
  <div style="display: table-cell">
    ...
    <div class="pagination">
      <g:if test="${datoHMGInstanceTotal > maxDisplay}">
        <g:paginate total="${datoHMGInstanceTotal}" action='showData' params="${returnParams}"/>
      </g:if>
  ...
  <div style="width:250px; height: 400px;display: table-cell; padding: 48px 0 0 36px;">
    <g:form method="post">
      ...
      <g:hiddenField name="paramId" value="${returnParams.paramId}" />
      <g:hiddenField name="dominio" value="${returnParams.dominio}" />

      <g:if test="${ pohms != null}">
        <g:if test="${pohms.size() <= 24}">
          <div style="margin: 8px 0 0 0; clear:both;">
            <label for="pohmid" style="width:60px">Pohm</label>
            <g:select name="pohmid" from="${pohms}" optionKey="id" value="${pohm?.id}" noSelection="['':'-TODOS-']"/>
          </div>
        </g:if>
        <g:if test="${pohms.size() > 24}">
          <div style="margin: 8px 0 0 0; clear:both;">
            <label for="pohmcdgo">Código POHM</label>
            <g:textField name="pohmcdgo" maxlength="16" value="${pohm?.codigo}"/>
          </div>
        </g:if>
      </g:if>
